...
public class Assist {
    ...
    public void delay(CodeActivity codeActivity) {
        Disposable disposable = Observable.just(1, 2, 3, 4, 5).timestamp().delay(1000, TimeUnit.MILLISECONDS)
                .subscribe(s -> Log.i("Assist", s + ""));
    }

    @Run(name = "切换线程（切换接收事件线程）")
    public void observerOn(CodeActivity codeActivity) {
        Disposable disposable = Observable.just(1, 2, 3, 4, 5).observeOn(Schedulers.newThread())
                .subscribe(s -> Log.i("Assist", s + ""));
    }

    /**
     * http://sherlockshi.github.io/2016/10/17/15_Android/1560_RxJava/RxJava%E6%93%8D%E4%BD%9C%E7%AC%A6%E5%AE%9E%E8
     * %B7%B5%EF%BC%9A6_%E8%BE%85%E5%8A%A9%E6%93%8D%E4%BD%9C%E4%B9%8B4_serialize/
     */
    @Run(name = "serialize \n一个Observable可以异步调用它的观察者的方法，可能是从不同的线程调用。这可能会让Observable行为不正确，它可能会在某一个onNext"
            + "调用之前尝试调用onCompleted或onError方法，或者从两个不同的线程同时调用onNext方法。使用Serialize操作符，你可以纠正这个Observable"
            + "的行为，保证它的行为是正确的且是同步的")
    public void serialize(CodeActivity codeActivity) {
        Disposable disposable =
                Observable.just(1, 3, 2, 4, 5, 8, 6, 7).serialize().subscribe(s -> Log.i("Assist", s + ""));
    }

    @Run(name = "切换线程（切换发出事件线程）")
    public void subscribeOn(CodeActivity codeActivity) {
        Disposable disposable = Observable.just(1, 3, 2, 4, 5, 8, 6, 7).subscribeOn(Schedulers.computation())
                .subscribe(s -> Log.i("Assist", s + ""));
    }
    ...
    public void timeInterval(CodeActivity codeActivity) {
        Disposable disposable =
    ...
    public void timeout(CodeActivity codeActivity) {
        Disposable disposable =
    ...
    public void timestamp(CodeActivity codeActivity) {
    ...
    public void doOnEach(CodeActivity codeActivity) {
        Disposable disposable = Observable.just(1, 2, 3, 4, 5)
                .doOnEach(integerNotification -> Log.i("Assist", integerNotification + "")).subscribe();
    }

    @Run(name = "doOnNext \n类似于doOnEach(Action1)，但是它的Action不是接受一个Notification参数，而是接受发射的数据项")
    public void doOnNext(CodeActivity codeActivity) {
        Disposable disposable =
    ...
    public void doOnSubscribe(CodeActivity codeActivity) {
    ...
    public void doOnDispose(CodeActivity codeActivity) {
        Disposable disposable = Observable.just(1, 2, 3, 4, 5).
    ...
    public void doOnCompleted(CodeActivity codeActivity) {
        Observable<Integer> observable = Observable.just(1, 2, 3, 4, 5).
                doOnComplete(() -> Log.i("Assist", "doOnComplete"));
    }

    @Run(name = "doOnError \n")
    public void doOnError(CodeActivity codeActivity) {
        Observable<Integer> observable = Observable.just(1, 2, 3, 4, 5).
                doOnNext(integer -> {
                    if (integer > 3) {
    ...
    public void doOnTerminate(CodeActivity codeActivity) {
        Observable<Integer> observable = Observable.just(1, 2, 3, 4, 5).
                doOnTerminate(() -> Log.i("Assist", "doOnTerminate"));
    }

    @Run(name = "doFinally \n注册一个动作，当它产生的Observable终止之后会被调用，无论是正常还是异常终止。")
    public void doFinally(CodeActivity codeActivity) {
        Observable<Integer> observable = Observable.just(1, 2, 3, 4, 5).
                doFinally(() -> Log.i("Assist", "doOnTerminate"));
    }

    @Run(name = "toList \n Observable将多项数据组合成一个List")
    public void toList(CodeActivity codeActivity) {
        Disposable disposable = Observable.just(1, 2, 3, 4, 5).
